...
//CORRIGIR: 
//		NO CASO DE EQUIPE PAR GANHOU / EQUIPE IMPAR GANHOU / EMPATE:
//		quem ta ganhando eh a impar, deve ser a par!!
				...
				if (jogada >= 49 && jogada <= 51) {
					...
					if ((LIS_ObterQtdElem(pCabecaMesa) - qtdCartasViradas) > 2) {
						...
						//JOGADOR EMPATOU COM ALGUMA CARTA DA MESA
							...
							//se for a ultima jogada quem joga na proxima é quem pôs a primeira carta q empatou
							if (qtdJogadas == qtdJogadores) {
								quemJoga = resultado ;
							} //fim if

							//se nao for a ultima jogada
							else {
								quemJoga = ProximoJogador(quemJoga, qtdJogadores) ;
							} //fim else
							atualJogadorVencedor = NINGUEM ;
						} //fim else if "jogador empatou com alguma carta da mesa"
...
for (i = 1; LIS_ObterQtdElem(pCabecaMesa)-1; i++) {
	...
	//CARTA APOSTADA != MANILHA
		...
		if (BAR_VerificarSeEhManilha(pCartaMesa, pCartaVira) == BAR_CondRetNaoEhManilha) {
			...
			//empataram
				...
				//verifica se eh a carta com a qual empatou perde pra alguma da mesa
				valorCartaQueEmpatou = valorCartaMesa ; 
				
				resultadoEmpate = CONSIDERAR ;

				LIS_IrInicioLista(pCabecaMesa) ;
				for (j = 1 ; i < LIS_ObterQtdElem(pCabecaMesa); j++ ) {
				...
				if (resultadoEmpate == CONSIDERAR) {
					...
					case 1: //empatou com quem jogou a primeira carta

						//se nao forem da mesma equipe
						if (quemJoga % 2 != quemJogouAPrimeira % 2) {
							return quemJogouAPrimeira ;
						}

						break;

					case 2: //empatou com quem jogou a segunda carta

						//se nao forem da mesma equipe
						if (quemJoga % 2 != quemJogouASegunda % 2) {
							return quemJogouASegunda ;
						}
						break;

					case 3: //empatou com quem jogou a terceira carta

						//se nao forem da mesma equipe
						if (quemJoga % 2 != quemJogouATerceira % 2) {
							return quemJogouATerceira ;
						}
						break;

					case 4: //empatou com quem jogou a quarta carta

						//se nao forem da mesma equipe
						if (quemJoga % 2 != quemJogouAQuarta % 2) {
							return quemJogouAQuarta ;
						}
						break;

					case 5: //empatou com quem jogou a quinta carta

						//se nao forem da mesma equipe
						if (quemJoga % 2 != quemJogouAQuinta % 2) {
							return quemJogouAQuinta ;
						}
						break;

					case 6: //empatou com quem jogou a sexta carta

						//se nao forem da mesma equipe
						if (quemJoga % 2 != quemJogouASexta % 2) {
							return quemJogouASexta ;
						}
						break;

					} //fim switch "ver com quem empatou"

				} //fim if "considerar empate"
